...
- 支持由本地运维台通过 URL fragment 一键覆盖 Binance 币种标记；fragment 不会发送到服务器，导入内容只写入当前浏览器
- 资金费率可在 8H 等效费率和 1Y 简单年化之间切换
- 资金费率后显示 Margin Pool 最新杠杆借币日利率的简单年化值，并支持排序
- Binance 活期可借币种在币对名后显示“（活期）”；名单从同源 Margin Pool 只读接口获取，并在浏览器持久缓存 1 小时
...
- 历史曲线提供独立的全量时间轴双滑块，默认框选最近 30 天，曲线横轴随选中时间窗口动态缩放
- 历史曲线用红点标出稳定结算周期的变化节点，悬停可查看调整前后的周期
...
- 运维台联动只接收带严格格式限制的币种列表，不接收账户、数量或会话令牌，也不提供公共写接口。
